...
class TestBadUsageError(DualEnv):
    def test_child_outside_parent(self):
        with self.assertRaises(UsageError), self.inv.inverse():
            pass

    def test_wrong_inversion(self):
        with self.env, self.assertRaises(UsageError), self.inv2:
            pass
    ...
    def test_inverses_are_siblings_not_same(self):
        with self.env, self.inv:
            ...
            with self.assertRaises(UsageError), inv_inv:
                pass

    def test_inv_direct(self):
        with self.assertRaises(UsageError), self.inv:
            pass
    ...
    def test_get_in_wrong_env(self):
        with self.env, self.assertRaises(UsageError):
            self.env2["unittest"]
        with self.env, self.assertRaises(UsageError):
            self.inv["unittest"]

    def test_inv2_get_in_inv(self):
        with self.env, self.inv, self.assertRaises(UsageError):
            self.inv2["unittest"]
